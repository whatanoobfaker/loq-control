				     ...
				     const struct fancurve *fancurve)
{
	...
	for (i = 0; i < FANCURVESIZE_IDEAPDAD; ++i) {
		...
		if (valr1 != point->rpm1_raw || valr2 != point->rpm2_raw)
			return -EIO;
...
static ssize_t fancurve_show(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	struct legion_private *priv = dev_get_drvdata(dev);
	struct fancurve fc;
	...
	for (i = 0; i < fc.size; ++i)
		len += sysfs_emit_at(buf, len, "%u %u %u %u\n",
				     fc.points[i].cpu_max_temp_celsius,
				     fc.points[i].gpu_max_temp_celsius,
				     fc.points[i].rpm1_raw * 100,
				     fc.points[i].rpm2_raw * 100);
	return len;
}
			      ...
			      size_t count)
{
	struct legion_private *priv = dev_get_drvdata(dev);
	struct fancurve fc;
	unsigned int cpu, gpu, rpm1, rpm2;
	unsigned int prev_cpu = 0, prev_rpm = 0;
	const char *p = buf;
	size_t i;
	int consumed;
	int err;
	...
	for (i = 0; i < fc.size; ++i) {
		if (sscanf(p, " %u %u %u %u%n", &cpu, &gpu, &rpm1, &rpm2,
			   &consumed) != 4) {
			err = -EINVAL;
			goto out;
		}
		p += consumed;
		if (cpu > 127 || gpu > 127 || rpm1 > 4500 || rpm2 > 4500 ||
		    (i == 0 && (rpm1 || rpm2)) || cpu < prev_cpu ||
		    max(rpm1, rpm2) < prev_rpm) {
			err = -EINVAL;
			goto out;
		}
		prev_cpu = cpu;
		prev_rpm = max(rpm1, rpm2);
		fc.points[i].cpu_max_temp_celsius = cpu;
		fc.points[i].gpu_max_temp_celsius = gpu;
		fc.points[i].rpm1_raw = rpm1 / 100;
		fc.points[i].rpm2_raw = rpm2 / 100;
	}

	err = write_fancurve(priv, &fc, false);
out:
	mutex_unlock(&priv->fancurve_mutex);
	return err ? err : count;
}

static DEVICE_ATTR_RW(fancurve);

static struct attribute *legion_sysfs_attributes[] = {
	&dev_attr_fancurve.attr,
...
static int __init legion_init(void)
{
	...
	ec = acpi_dev_get_first_match_dev("PNP0C09", NULL, -1);
	if (!ec) {
		platform_driver_unregister(&legion_driver);
		return -ENODEV;
	}
	info.fwnode = acpi_fwnode_handle(ec);
	legion_pdev = platform_device_register_full(&info);
	acpi_dev_put(ec);
